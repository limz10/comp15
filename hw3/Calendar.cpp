...
// The addEvents() method needs to create a Day 
// if one does not exist for a particular event,
// and it needs to create events in that Day.
...
//	cerr << eventLine << endl;
	...
	if (tempDay->scheduleEarliest(evt) == false) 
	     // could not schedule the event!
	    cout << "Could not schedule the event: " << evt.eventCSV() << endl;	
    }
}

// getDay() returns the pointer in dayList of startDate. 
// If the Day does not exist, the function returns NULL
...
// This function returns a pointer to the Day 
// so other functions can insert events into that Day.
Day *Calendar::insertDay(date startDate) 
{
	...
	if (startDate.year < dayList[index].getDate().year) break;
	if (startDate.year == dayList[index].getDate().year) {
	    if (startDate.month < dayList[index].getDate().month) break;
	    else if (startDate.month == dayList[index].getDate().month)
		if (startDate.day < dayList[index].getDate().day) break;
	}
    }
    for (int moveIndex = dayCount; moveIndex > index; moveIndex--) 
	dayList[moveIndex] = dayList[moveIndex - 1];

    dayList[index] = *newDay;
    dayCount++;
